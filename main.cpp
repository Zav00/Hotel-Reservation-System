...
void generateBill(const Reservation &reservation) {
    ...
    // Add relevant costs calculation here
    std::cout << "----------------------\n";
}

void saveBillToFile(const Reservation &reservation) {
    std::ofstream outputFile("bill.txt", std::ios::app);
...
int main() {
    ...
    registerCustomer(customers, "zz", "ass.@mail.ru");
    registerCustomer(customers, "zz", "ass.com");
    bookRoom(rooms, customers, reservations, "Single", 101, "J", "j.d@.com");
    bookRoom(rooms, customers, reservations, "Double", 202, "D", "d.@.com");
